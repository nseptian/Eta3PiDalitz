void EtaTo3PiReconstructionInd(int data_set,TString outName,bool is_mc){
  ...
  //kfit_cut2 = 0.1;
    ...
    Double_t kfit_cut = 0.00; //This values represents a cut on the kinematic fit probability!
    ...
    for(Int_t i=0;i<nEntries;i++){
        ...
        if(is_truecombo && kfit_prob > kfit_cut){
            ...
            // sum_amp_2 += (amp_2);
            //--------------------------- 
            if ((theta_photon1*TMath::RadToDeg() < 10.3 || theta_photon1*TMath::RadToDeg() > 11.9) && (theta_photon2*TMath::RadToDeg() < 10 || theta_photon2*TMath::RadToDeg() > 11.9)) out_tree->Fill();
