void EtaTo3PiReconstructionAll(int data_set,TString outName,bool is_mc){
  ...
  //kfit_cut2 = 0.1;
    ...
    //   TTree *dataChain = (TTree*)in->Get("myTree");
    // }
    ...
    TH1F *hpippimg1g2mass = new TH1F("h_pippimg1g2mass","",101,0.5,0.6);
    ...
    for(Int_t i=0;i<nEntries;i++){
        ...
        if(is_truecombo && kfit_prob > kfit_cut){
            ...
            PyP1 = pip_p4_kin->Py();
            PzP1 = pip_p4_kin->Pz();
            Double_t pip_momentum = TMath::Sqrt(TMath::Power(PxP1,2) + TMath::Power(PyP1,2) + TMath::Power(PzP1,2));
            
            EnP2 = pim_p4_kin->E();
            PxP2 = pim_p4_kin->Px();
            PyP2 = pim_p4_kin->Py();
            PzP2 = pim_p4_kin->Pz();
            Double_t pim_momentum = TMath::Sqrt(TMath::Power(PxP2,2) + TMath::Power(PyP2,2) + TMath::Power(PzP2,2));
            ...
            // sum_amp_2 += (amp_2);
            //--------------------------- 
            if (m_pi0 > 0.11 && m_pi0 < 0.165 && (theta_photon1*TMath::RadToDeg() < 10.3 || theta_photon1*TMath::RadToDeg() > 11.9) && (theta_photon2*TMath::RadToDeg() < 10.3 || theta_photon2*TMath::RadToDeg() > 11.9)
                && (pip_momentum > 0.7) && (pim_momentum > 0.7)) {
              out_tree->Fill();
